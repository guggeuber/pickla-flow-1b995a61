-- Customer Performance V2 Phase D repair: keep the public Prices projection
-- set-based while sourcing the generic First Visit price from a concrete,
-- standard public Open Play occurrence. The previous migration is immutable.
-- Checkout, entitlements and frozen financial truth remain intentionally absent.
...
AS $$
  ...
  -- A generic First Visit presentation needs an authoritative occurrence, not
  -- the zero-valued product template. "standard" is the existing operator
  -- distinction between ordinary Open Play and promotional/special sessions.
  -- Returning the whole fact lets the shared resolver retain price precedence,
  -- including an explicitly free (zero-priced) standard occurrence.
  first_visit_fallback_occurrence AS MATERIALIZED (
    SELECT session.id, session.venue_id, session.name, session.session_type,
      session.session_date, session.recurrence_days, session.start_time,
      session.end_time, session.capacity, session.price_sek, session.product_key,
      session.access_policy, session.metadata, session.early_bird_price_minor,
    ...
    WHERE session.is_active = true
      AND session.publish_status = 'published'
      AND session.closed_to_public = false
      AND session.first_visit_offer_enabled = false
      AND session.session_type = 'open_play'
      AND CASE
        WHEN session.product_key IS NOT NULL AND session.product_key <> 'day_access'
          THEN session.product_key
        WHEN session.session_type = 'open_play' THEN 'open_play_slot'
        ...
        ELSE 'session_ticket'
      END = 'open_play_slot'
      AND COALESCE(NULLIF(BTRIM(session.metadata->>'pricing_channel_mode'), ''), 'standard') = 'standard'
      AND (
  ...
  first_visit_pricing_occurrences AS MATERIALIZED (
    SELECT occurrence.*, false AS is_fallback
    FROM first_visit_occurrences occurrence
    UNION ALL
    SELECT occurrence.*, true AS is_fallback
    FROM first_visit_fallback_occurrence occurrence
  ),
  first_visit_registration_counts AS (
    ...
    FROM first_visit_pricing_occurrences occurrence
  ...
  first_visit_json_rows AS (
    SELECT activity.is_fallback, activity.occurrence_date, activity.start_time, activity.id,
      jsonb_build_object(
  ...
  SELECT jsonb_build_object(
    ...
    'first_visit_fallback_occurrence', (
      SELECT activity.value
      FROM first_visit_json_rows activity
      WHERE activity.is_fallback = true
      ORDER BY activity.occurrence_date, activity.start_time, activity.id
      LIMIT 1
    ),
    'first_visit_occurrences', COALESCE((
      SELECT jsonb_agg(activity.value ORDER BY activity.occurrence_date, activity.start_time, activity.id)
      FROM first_visit_json_rows activity
      WHERE activity.is_fallback = false
    ), '[]'::JSONB)
  );
$$;

COMMENT ON FUNCTION public.public_customer_prices_facts(TEXT, DATE, DATE, TIMESTAMPTZ) IS
  'Bounded auth-free Prices presentation facts. Generic First Visit pricing is projected from a standard public Open Play occurrence and resolved in Edge; checkout, personalized pricing, entitlements and frozen Order truth remain canonical.';
