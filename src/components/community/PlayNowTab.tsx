    ...
    <motion.div variants={container} initial="hidden" animate="show" className="flex flex-col gap-6 pb-8">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-3">
          {tiers.map((tier, i) => {
            const isAccent = i === Math.floor(tiers.length / 2); // middle tier gets accent
            return (
              <motion.div
                key={tier.id}
                variants={item}
                  ...
                  <div className="text-right flex-shrink-0 ml-3">
                    {tier.monthly_price != null && tier.monthly_price > 0 && (
                      <p
                        ...
                        style={{ fontFamily: FONT_MONO, color: isAccent ? "#fff" : tier.color || "#E86C24" }}
                      >
                    ...
                    {tier.dayPassPrice != null && (
                      ...
                      >
                        {tier.dayPassLabel || "Dagspass"}: {Math.round(tier.dayPassPrice)} kr
                      </p>
                    )}
                  </div>
                </div>
        ...
        >
          <p className="text-sm" style={{ color: "rgba(62,61,57,0.4)", fontFamily: FONT_MONO }}>
            Inga medlemskap konfigurerade ännu
          </p>
        </motion.div>
      )}
