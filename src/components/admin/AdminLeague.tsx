...
import { AlertTriangle, CalendarDays, Check, Loader2, Pencil, Save, Users } from "lucide-react";
...
function CreateLeague({ venueId, onDone }: { venueId: string; onDone?: () => void }) {
  ...
  const setNightDate = (index: number, value: string) => {
    setDates((current) => {
      if (index === 0 && value && current.slice(1).every((date) => !date)) {
        const first = DateTime.fromISO(value, { zone: "Europe/Stockholm" });
        if (first.isValid) return Array.from({ length: 5 }, (_, offset) => first.plus({ weeks: offset }).toISODate()!);
      }
      return current.map((date, dateIndex) => dateIndex === index ? value : date);
    });
  };
  const create = useMutation({
    mutationFn: () => createLeagueSeason({
      venue_id: venueId,
      name,
      description,
    ...
    onSuccess: async () => {
      toast.success(publish ? "Seriespelet är publicerat" : "Seriespelet är skapat som utkast");
      await queryClient.invalidateQueries({ queryKey: ["admin-leagues", venueId] });
      onDone?.();
    },
    onError: (error: Error) => toast.error(error.message),
  });
  const thursdayValid = dates.every((date) => date && DateTime.fromISO(date).weekday === 4)
  ...
  return <div className="space-y-5">
    <section className="rounded-2xl p-4" style={{ background: ax("surfaceHi"), border: `1px solid ${ax("borderSoft")}` }}><p className="font-mono text-[10px] font-black uppercase tracking-[0.2em]" style={{ color: ax("magenta") }}>Innehåll</p><div className="mt-3 grid gap-3"><input aria-label="Titel" value={name} onChange={(event) => setName(event.target.value)} className={inputClass} style={{ borderColor: ax("border") }} /><textarea aria-label="Beskrivning" value={description} onChange={(event) => setDescription(event.target.value)} rows={3} className="rounded-xl border bg-transparent p-3 text-sm text-white" style={{ borderColor: ax("border") }} /><input aria-label="Bild-URL" value={imageUrl} onChange={(event) => setImageUrl(event.target.value)} placeholder="Artwork URL (https://...)" className={inputClass} style={{ borderColor: ax("border") }} /></div></section>
    <section className="rounded-2xl p-4" style={{ background: ax("surfaceHi"), border: `1px solid ${ax("borderSoft")}` }}><p className="font-mono text-[10px] font-black uppercase tracking-[0.2em]" style={{ color: ax("electricSoft") }}>Säsong · låst V1</p><div className="mt-3 grid gap-3"><div className="grid grid-cols-1 gap-2 sm:grid-cols-5">{dates.map((date, index) => <label key={index} className="grid gap-1 text-[10px] font-bold" style={{ color: ax("muted") }}>Torsdag {index + 1}<input type="date" value={date} onChange={(event) => setNightDate(index, event.target.value)} className={inputClass} style={{ borderColor: ax("border") }} /></label>)}</div>{dates.some(Boolean) && !thursdayValid ? <p className="text-xs font-bold" style={{ color: ax("danger") }}>Välj fem unika torsdagar i stigande ordning.</p> : null}<p className="text-[10px]" style={{ color: ax("muted") }}>Första datumet föreslår fem veckor. Varje exakt torsdag kan därefter justeras.</p><div className="grid grid-cols-2 gap-2 text-xs font-bold text-white"><div className="rounded-xl p-3" style={{ background: ax("surface") }}>18:00–20:00</div><div className="rounded-xl p-3" style={{ background: ax("surface") }}>18:00 + 19:00 · 50 min</div><div className="rounded-xl p-3" style={{ background: ax("surface") }}>6 lag · 2 spelare</div><div className="rounded-xl p-3" style={{ background: ax("surface") }}>2 matcher/lag/kväll</div></div></div></section>
    ...
    <button type="button" onClick={() => create.mutate()} disabled={!ready || create.isPending} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl font-black disabled:opacity-40" style={{ background: ax("electric"), color: ax("ink") }}>{create.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Skapa Seriespel</button>
  </div>;
}
