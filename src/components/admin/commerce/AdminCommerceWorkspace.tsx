...
import {
  ...
  ClipboardCheck, ExternalLink, History, Image as ImageIcon, Loader2, Package,
...
function emptyDraft(): ProductDraft {
  return {
    ...
    category: "", sport: "", imageUrl: "", commerceKind: "merchandise",
...
function draftFromProduct(product: AdminCommerceProduct): ProductDraft {
  return {
    ...
    category: product.category || "", sport: product.sport || "", imageUrl: product.image_url || "",
...
function OrdersWorkspace({ venueId, operations, refresh, productId, onLoadMore, loadingMore }: { venueId: string; operations: InventoryOperations; refresh: () => Promise<void>; productId?: string; onLoadMore?: () => void; loadingMore?: boolean }) {
  ...
  const orders = useMemo(() => operations.orders.filter((order) => {
    ...
    return [order.id, order.guest_name, receiptFor(order)?.receipt_number, ...order.lines.flatMap((line) => [line.product_name, line.sku])].filter(Boolean).some((value) => String(value).toLocaleLowerCase("sv-SE").includes(needle));
  }), [operations.orders, productId, search, status, fulfillment]);
  return <div className="space-y-4" data-testid="admin-commerce-orders"><div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_150px_180px]"><div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2" style={{ color: ax("muted") }} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Sök order, kvitto, kund eller SKU" className={`${INPUT} pl-9`} style={{ borderColor: ax("border"), background: ax("surfaceHi") }} /></div><select aria-label="Orderstatus" value={status} onChange={(event) => setStatus(event.target.value)} className={INPUT} style={{ borderColor: ax("border"), background: ax("surfaceHi") }}><option value="all">Alla orderstatusar</option><option value="paid">Betald</option><option value="checkout_pending">Checkout pågår</option><option value="attention">Behöver åtgärd</option><option value="cancelled">Avbruten</option><option value="expired">Utgången</option></select><select aria-label="Utlämningsstatus" value={fulfillment} onChange={(event) => setFulfillment(event.target.value)} className={INPUT} style={{ borderColor: ax("border"), background: ax("surfaceHi") }}><option value="all">Alla utlämningsstatusar</option><option value="pending_pickup">Väntar på uthämtning</option><option value="collected">Uthämtad</option><option value="attention">Behöver åtgärd</option><option value="not_collected">Ej uthämtad</option></select></div>
    {orders.length ? orders.map((order) => {
      const receipt = receiptFor(order);
      const open = expanded === order.id;
      const tracked = order.lines.some((line) => line.inventory_policy === "tracked");
      return <article key={order.id} className="overflow-hidden rounded-2xl" style={PANEL}><button type="button" onClick={() => setExpanded(open ? null : order.id)} className="grid w-full gap-3 p-4 text-left sm:grid-cols-[minmax(0,1fr)_130px_130px_auto] sm:items-center"><span><span className="flex flex-wrap items-center gap-2"><span className="text-sm font-black text-white">{receipt?.receipt_number || `Order ${order.id.slice(0, 8)}`}</span><StatusChip tone={order.status === "paid" ? "lime" : order.status === "attention" ? "danger" : order.status === "checkout_pending" ? "sun" : "neutral"}>{order.status.replaceAll("_", " ")}</StatusChip>{tracked ? <StatusChip tone="electric">Lagerspårad</StatusChip> : null}</span><span className="mt-1 block text-[10px]" style={{ color: ax("muted") }}>{order.guest_name || "Kund"} · {DateTime.fromISO(order.created_at).setZone("Europe/Stockholm").toFormat("yyyy-LL-dd HH:mm")}</span></span><span className="text-xs text-white">{order.lines.length} orderrad{order.lines.length === 1 ? "" : "er"}</span><span className="text-sm font-black text-white">{sekFromMinor(order.total_inc_vat_minor)}</span><ChevronDown className={`h-4 w-4 transition-transform ${open ? "rotate-180" : ""}`} style={{ color: ax("muted") }} /></button>{open ? <div className="border-t p-4" style={{ borderColor: ax("borderSoft") }}><div className="grid gap-2 sm:grid-cols-3"><Metric label="Totalt inkl. moms" value={sekFromMinor(order.total_inc_vat_minor)} /><Metric label="Moms" value={sekFromMinor(order.vat_amount_minor)} /><Metric label="Betalning" value={receipt?.payment_status || order.status} /></div><div className="mt-4 space-y-3">{order.lines.map((line) => <div key={line.id} className="rounded-xl p-3" style={{ background: ax("surface") }}><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-xs font-black text-white">{line.product_name} × {line.quantity}</p><p className="mt-0.5 font-mono text-[9px]" style={{ color: ax("muted") }}>{line.sku ? `SKU ${line.sku} · ` : ""}{String(line.variant_snapshot?.title || "")}</p></div><div className="text-right"><p className="text-xs font-black text-white">{sekFromMinor(line.line_total_inc_vat_minor)}</p><p className="text-[9px]" style={{ color: ax("muted") }}>{line.vat_rate}% moms · {line.fulfillment_status.replaceAll("_", " ")}</p></div></div>{line.fulfillment_type === "desk_pickup" && !["collected", "not_collected"].includes(line.fulfillment_status) ? <Link to="/desk" className="mt-2 inline-flex items-center gap-1 text-[10px] font-black" style={{ color: ax("electricSoft") }}>Hantera uthämtning i Desk <ExternalLink className="h-3 w-3" /></Link> : null}<RefundAndReturn venueId={venueId} order={order} line={line} operations={operations} refresh={refresh} /></div>)}</div>{operations.dispositions.some((item) => item.commerce_order_id === order.id) ? <div className="mt-3"><p className="font-mono text-[9px] font-black uppercase tracking-[0.15em]" style={{ color: ax("muted") }}>Fysisk historik</p>{operations.dispositions.filter((item) => item.commerce_order_id === order.id).map((item) => <p key={item.id} className="mt-1 text-[10px]" style={{ color: ax("muted") }}>{item.outcome.replaceAll("_", " ")} × {item.quantity} · {DateTime.fromISO(item.created_at).setZone("Europe/Stockholm").toFormat("yyyy-LL-dd HH:mm")}</p>)}</div> : null}</div> : null}</article>;
    ...
    {operations.orders_page?.has_more && onLoadMore ? <button type="button" disabled={loadingMore} onClick={onLoadMore} className="w-full rounded-xl px-4 py-3 text-xs font-black disabled:opacity-40" style={{ border: `1px solid ${ax("border")}`, color: "white" }}>{loadingMore ? "Hämtar…" : "Ladda äldre ordrar"}</button> : null}
  </div>;
}
...
function ProductWizard({ venueId, products, onCancel, onCreated }: { venueId: string; products: AdminCommerceProduct[]; onCancel: () => void; onCreated: (product: AdminCommerceProduct) => void }) {
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const saved = await apiPost<AdminCommerceProduct>("api-admin", "products", {
        ...
        image_url: draft.imageUrl.trim() || null, commerce_kind: draft.commerceKind,
  ...
  return <div className="space-y-4" data-testid="commerce-product-wizard"><div className="flex items-center gap-3"><button type="button" onClick={onCancel} className="grid h-10 w-10 place-items-center rounded-xl" style={{ background: ax("surfaceHi"), color: "white" }} aria-label="Avbryt produkt"><ArrowLeft className="h-4 w-4" /></button><div><p className="font-mono text-[10px] font-black uppercase tracking-[0.2em]" style={{ color: ax("magenta") }}>Ny produkt · steg {step} av 4</p><h2 className="font-display text-xl font-black text-white">{step === 1 ? "Välj produkttyp" : step === 2 ? "Produktinformation" : step === 3 ? "Varianter och SKU" : "Granska utkast"}</h2></div></div>
    ...
    {step === 2 ? <div className="grid gap-4 lg:grid-cols-2"><section className="space-y-3 rounded-2xl p-4" style={PANEL}><h3 className="text-sm font-black text-white">Grunduppgifter</h3><Field label="Namn"><input value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} className={INPUT} style={{ borderColor: ax("border") }} /></Field><Field label="Beskrivning"><textarea rows={4} value={draft.description} onChange={(event) => setDraft((current) => ({ ...current, description: event.target.value }))} className={INPUT} style={{ borderColor: ax("border") }} /></Field><div className="grid grid-cols-2 gap-3"><Field label="Pris SEK"><input type="number" min="0" value={draft.price} onChange={(event) => setDraft((current) => ({ ...current, price: event.target.value }))} className={INPUT} style={{ borderColor: ax("border") }} /></Field><Field label="Moms %"><input type="number" min="0" max="100" value={draft.vatRate} onChange={(event) => setDraft((current) => ({ ...current, vatRate: event.target.value }))} className={INPUT} style={{ borderColor: ax("border") }} /></Field></div><div className="grid grid-cols-2 gap-3"><Field label="Kategori"><input value={draft.category} onChange={(event) => setDraft((current) => ({ ...current, category: event.target.value }))} placeholder="Kläder" className={INPUT} style={{ borderColor: ax("border") }} /></Field><Field label="Sport"><input value={draft.sport} onChange={(event) => setDraft((current) => ({ ...current, sport: event.target.value }))} placeholder="Pickleball" className={INPUT} style={{ borderColor: ax("border") }} /></Field></div><Field label="Bildlänk"><div className="relative"><ImageIcon className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2" style={{ color: ax("muted") }} /><input value={draft.imageUrl} onChange={(event) => setDraft((current) => ({ ...current, imageUrl: event.target.value }))} placeholder="https://…" className={`${INPUT} pl-9`} style={{ borderColor: ax("border") }} /></div></Field></section><section className="space-y-4 rounded-2xl p-4" style={PANEL}><h3 className="text-sm font-black text-white">Försäljning och leverans</h3>{draft.inventoryPolicy === "tracked" ? <><TrackedGate /><p className="text-xs" style={{ color: ax("muted") }}>Fristående shop · hämtas i receptionen · aktivitetstillval stöds inte för spårad vara i R2A.</p></> : <><label className="flex items-center justify-between gap-4"><span><span className="block text-sm font-bold text-white">Fristående shop</span><span className="text-xs" style={{ color: ax("muted") }}>Kan köpas utan aktivitet.</span></span><Switch checked={draft.standaloneEnabled} onCheckedChange={(checked) => setDraft((current) => ({ ...current, standaloneEnabled: checked }))} /></label><label className="flex items-center justify-between gap-4"><span><span className="block text-sm font-bold text-white">Aktivitetstillval</span><span className="text-xs" style={{ color: ax("muted") }}>Relation väljs efter att produkten skapats.</span></span><Switch checked={draft.activityAddonEnabled} onCheckedChange={(checked) => setDraft((current) => ({ ...current, activityAddonEnabled: checked }))} /></label></>}<Field label="Leverans"><select value={draft.fulfillment} disabled={draft.inventoryPolicy === "tracked"} onChange={(event) => setDraft((current) => ({ ...current, fulfillment: event.target.value as ProductDraft["fulfillment"] }))} className={INPUT} style={{ borderColor: ax("border") }}><option value="desk_pickup">Hämtas i receptionen</option><option value="digital">Digital</option><option value="participation">Deltagande</option></select></Field>{draft.inventoryPolicy === "tracked" ? <Field label="Lager- och utlämningsplats"><input value={draft.locationName} onChange={(event) => setDraft((current) => ({ ...current, locationName: event.target.value }))} className={INPUT} style={{ borderColor: ax("border") }} /></Field> : null}</section></div> : null}
    ...
    {step === 4 ? <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]"><section className="rounded-2xl p-5" style={PANEL}><div className="flex items-start gap-4"><span className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-xl" style={{ background: ax("surface") }}>{draft.imageUrl ? <img src={draft.imageUrl} alt="" className="h-full w-full object-cover" /> : <ShoppingBag className="h-5 w-5" style={{ color: ax("muted") }} />}</span><div><h3 className="text-lg font-black text-white">{draft.name}</h3><p className="mt-1 text-xs" style={{ color: ax("muted") }}>{draft.description || "Ingen beskrivning"}</p><div className="mt-3 flex flex-wrap gap-2"><StatusChip tone="sun">Utkast</StatusChip><StatusChip tone={draft.inventoryPolicy === "tracked" ? "electric" : "neutral"}>{draft.inventoryPolicy === "tracked" ? `${matrix.length} lagerspårade varianter` : "Utan lagersaldo"}</StatusChip></div></div></div><div className="mt-5 grid grid-cols-2 gap-2"><Metric label="Baspris" value={sek(Number(draft.price))} /><Metric label="Moms" value={`${draft.vatRate}%`} /><Metric label="Kanal" value={draft.inventoryPolicy === "tracked" || draft.standaloneEnabled ? "Fristående shop" : draft.activityAddonEnabled ? "Aktivitet" : "Ej till salu"} /><Metric label="Leverans" value={draft.inventoryPolicy === "tracked" || draft.fulfillment === "desk_pickup" ? "Hämtas i receptionen" : draft.fulfillment} /></div></section><aside className="space-y-3"><TrackedGate /><button type="button" onClick={() => create.mutate()} disabled={create.isPending} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl text-sm font-black" style={{ background: `linear-gradient(135deg, ${ax("electric")}, ${ax("magenta")})`, color: "white" }}>{create.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} Skapa säkert utkast</button><p className="text-center text-[10px]" style={{ color: ax("muted") }}>Inget lagersaldo skapas. Ingen spårad försäljning aktiveras.</p></aside></div> : null}
    <div className="flex items-center justify-between"><button type="button" onClick={() => step === 1 ? onCancel() : setStep((value) => value - 1)} className="rounded-xl px-4 py-2.5 text-xs font-black" style={{ border: `1px solid ${ax("border")}`, color: ax("muted") }}>{step === 1 ? "Avbryt" : "Tillbaka"}</button>{step < 4 ? <button type="button" disabled={!canContinue} onClick={() => setStep((value) => value + 1)} className="rounded-xl px-4 py-2.5 text-xs font-black disabled:opacity-40" style={{ background: ax("electric"), color: ax("ink") }}>Fortsätt</button> : null}</div>
  </div>;
}
