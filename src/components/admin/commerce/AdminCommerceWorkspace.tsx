...
import {
  activityTicketProductFields, buildVariantMatrix, normalizeCode, productInventoryState, sek, sekFromMinor,
...
import StorefrontPresentationEditor from "@/components/admin/commerce/StorefrontPresentationEditor";
...
type ProductDraft = {
  ...
  category: string;
  sport: string;
  commerceKind: AdminProductCreationKind;
  sessionType: string;
...
function emptyDraft(): ProductDraft {
  return {
    ...
    sessionType: "group_training", founderDiscount: "20",
...
function draftFromProduct(product: AdminCommerceProduct): ProductDraft {
  return {
    ...
    commerceKind: product.commerce_kind === "participation" ? "participation" : product.commerce_kind === "rental" ? "rental" : "merchandise",
    sessionType: product.session_type || "group_training",
...
function ProductWizard({ venueId, products, onCancel, onCreated }: { venueId: string; products: AdminCommerceProduct[]; onCancel: () => void; onCreated: (product: AdminCommerceProduct) => void }) {
  ...
  const participation = draft.commerceKind === "participation";
  const membershipTiersQuery = useQuery<Array<{ id: string; name: string; is_active: boolean }>>({
    queryKey: ["membership-tiers", venueId],
    queryFn: () => apiGet("api-memberships", "tiers", { venueId, includeHidden: "true" }),
    enabled: participation,
  });
  const membershipTiers = membershipTiersQuery.data || [];
  const founderTier = membershipTiers.find((tier) => tier.is_active && tier.name.trim().toLocaleLowerCase("sv-SE") === "founder") || null;
  const create = useMutation({
    mutationFn: async () => {
      ...
      if (!Number.isFinite(Number(draft.vatRate)) || Number(draft.vatRate) < 0 || Number(draft.vatRate) > 100) throw new Error("Ange en giltig momssats.");
      const founderDiscount = Number(draft.founderDiscount || 0);
      if (participation && (!Number.isFinite(founderDiscount) || founderDiscount < 0 || founderDiscount > 100)) throw new Error("Founder-rabatten måste vara 0–100 %.");
      if (participation && founderDiscount > 0 && !founderTier) throw new Error("Ingen aktiv Founder-nivå hittades. Skapa eller aktivera Founder innan produkten sparas.");
      ...
      const activityFields = participation ? activityTicketProductFields(draft.sessionType) : {};
      const saved = await apiPost<AdminCommerceProduct>("api-admin", "products", {
        ...
        fulfillment_presentation: tracked ? "desk_pickup" : participation ? "participation" : draft.fulfillment,
        category: draft.category.trim() || null, sport: draft.sport.trim() || null,
        image_url: null, commerce_kind: draft.commerceKind, ...activityFields,
      ...
      if (!tracked) {
        ...
        if (participation) {
          if (founderDiscount > 0) {
            await apiPost("api-memberships", "tier-pricing", {
              tierId: founderTier.id,
              product_type: saved.product_key,
              fixed_price: null,
              discount_percent: founderDiscount,
              vat_rate: Number(draft.vatRate),
              label: saved.name,
              allow_draft_product: true,
            });
          }
          return apiPatch<AdminCommerceProduct>("api-admin", "products", {
            venueId,
            productId: saved.id,
            status: "active",
          });
    ...
    onSuccess: async (saved) => { await queryClient.invalidateQueries({ queryKey: ["admin-access-products", venueId] }); toast.success(participation ? "Aktivitetsprodukten skapades och kan väljas i schemat." : "Produktutkastet skapades. Spårad försäljning är fortfarande avstängd."); onCreated(saved); },
  ...
  return <div className="space-y-4" data-testid="commerce-product-wizard"><div className="flex items-center gap-3"><button type="button" onClick={onCancel} className="grid h-10 w-10 place-items-center rounded-xl" style={{ background: ax("surfaceHi"), color: "white" }} aria-label="Avbryt produkt"><ArrowLeft className="h-4 w-4" /></button><div><p className="font-mono text-[10px] font-black uppercase tracking-[0.2em]" style={{ color: ax("magenta") }}>Ny produkt · steg {step} av 4</p><h2 className="font-display text-xl font-black text-white">{step === 1 ? "Välj produkttyp" : step === 2 ? "Produktinformation" : step === 3 ? "Varianter och SKU" : "Granska utkast"}</h2></div></div>
    ...
    {step === 1 ? <div className="grid gap-3 sm:grid-cols-3">{([['participation', 'Aktivitetsbiljett / tjänst', 'Ett bokningsbart pass eller en tjänst, till exempel Singelträning.'], ['merchandise', 'Fysisk vara', 'T-shirts, hoodies, kepsar och framtida tillbehör.'], ['rental', 'Uthyrning', 'Befintliga produkter som Hyr rack fortsätter utan lagersaldo.']] as const).map(([kind, title, body]) => <button key={kind} type="button" onClick={() => setDraft((current) => ({ ...current, commerceKind: kind, inventoryPolicy: kind === "merchandise" ? current.inventoryPolicy : "stockless", fulfillment: kind === "participation" ? "participation" : "desk_pickup", standaloneEnabled: kind === "participation" ? false : current.standaloneEnabled, activityAddonEnabled: kind === "participation" ? false : current.activityAddonEnabled, vatRate: kind === "participation" ? "6" : current.vatRate }))} className="rounded-2xl p-5 text-left" style={{ ...PANEL, borderColor: draft.commerceKind === kind ? ax("electric") : ax("borderSoft") }}><ShoppingBag className="h-5 w-5" style={{ color: kind === "merchandise" ? ax("magenta") : ax("electricSoft") }} /><h3 className="mt-3 text-base font-black text-white">{title}</h3><p className="mt-1 text-xs leading-relaxed" style={{ color: ax("muted") }}>{body}</p></button>)}</div> : null}
    ...
    {step === 2 ? <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="space-y-4 rounded-2xl p-4" style={PANEL}>
          ...
          {participation ? <><p className="text-xs leading-relaxed" style={{ color: ax("muted") }}>Produkten säljs som deltagande i ett konkret pass. Den blir valbar under “Vad säljer vi?” när den har aktiverats.</p><Field label="Aktivitetstyp"><select value={draft.sessionType} onChange={(event) => setDraft((current) => ({ ...current, sessionType: event.target.value }))} className={INPUT} style={{ borderColor: ax("border") }}><option value="group_training">Träning (singel eller grupp)</option><option value="open_play">Open Play</option><option value="club_night">Klubbkväll</option><option value="pickla_open">Pickla Open</option><option value="event">Event</option></select></Field><Field label="Founder-rabatt %" hint="kanonisk medlemsprisregel"><input type="number" min="0" max="100" value={draft.founderDiscount} onChange={(event) => setDraft((current) => ({ ...current, founderDiscount: event.target.value }))} className={INPUT} style={{ borderColor: ax("border") }} /></Field><div className="rounded-xl p-3 text-xs leading-relaxed" style={{ background: ax("surface"), color: ax("muted") }}><span className="font-black text-white">Access:</span> Ingår inte automatiskt i Day, Play eller Play+. Inkludering väljs på passet.<br /><span className="font-black text-white">Avbokning:</span> Standard 12h via Policy V1 för occurrence ticket.</div></> : draft.inventoryPolicy === "tracked" ? <><TrackedGate /><p className="text-xs" style={{ color: ax("muted") }}>Fristående shop · hämtas i receptionen · aktivitetstillval stöds inte för spårad vara i R2A.</p><Field label="Utlämningsplats"><input value={draft.locationName} onChange={(event) => setDraft((current) => ({ ...current, locationName: event.target.value }))} className={INPUT} style={{ borderColor: ax("border") }} /></Field></> : <><label className="flex items-center justify-between gap-4"><span><span className="block text-sm font-bold text-white">Fristående shop</span><span className="text-xs" style={{ color: ax("muted") }}>Kan köpas utan aktivitet.</span></span><Switch checked={draft.standaloneEnabled} onCheckedChange={(checked) => setDraft((current) => ({ ...current, standaloneEnabled: checked }))} /></label><label className="flex items-center justify-between gap-4"><span><span className="block text-sm font-bold text-white">Aktivitetstillval</span><span className="text-xs" style={{ color: ax("muted") }}>Relation väljs efter att produkten skapats.</span></span><Switch checked={draft.activityAddonEnabled} onCheckedChange={(checked) => setDraft((current) => ({ ...current, activityAddonEnabled: checked }))} /></label></>}
        </section>
      </div>
      <details className="rounded-2xl p-4" style={PANEL}><summary className="cursor-pointer text-sm font-black text-white">Avancerat</summary><p className="mt-1 text-xs" style={{ color: ax("muted") }}>Kategori, sport och ovanliga leveranssätt.</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><Field label="Kategori"><input value={draft.category} onChange={(event) => setDraft((current) => ({ ...current, category: event.target.value }))} placeholder={participation ? "Träning" : "Kläder"} className={INPUT} style={{ borderColor: ax("border") }} /></Field><Field label="Sport"><input value={draft.sport} onChange={(event) => setDraft((current) => ({ ...current, sport: event.target.value }))} placeholder="Pickleball" className={INPUT} style={{ borderColor: ax("border") }} /></Field><Field label="Leverans"><select value={draft.fulfillment} disabled={draft.inventoryPolicy === "tracked" || participation} onChange={(event) => setDraft((current) => ({ ...current, fulfillment: event.target.value as ProductDraft["fulfillment"] }))} className={INPUT} style={{ borderColor: ax("border") }}><option value="desk_pickup">Hämtas i receptionen</option><option value="digital">Digital</option><option value="participation">Deltagande</option></select></Field></div></details>
    </div> : null}
    ...
    {step === 4 ? <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]"><section className="rounded-2xl p-5" style={PANEL}><div className="flex items-start gap-4"><span className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-xl" style={{ background: ax("surface") }}>{pendingImages[0] ? <img src={pendingImages[0].preview} alt={pendingImages[0].altText} className="h-full w-full object-cover" /> : <ShoppingBag className="h-5 w-5" style={{ color: ax("muted") }} />}</span><div><h3 className="text-lg font-black text-white">{draft.name}</h3><p className="mt-1 text-xs" style={{ color: ax("muted") }}>{draft.description || "Ingen beskrivning"}</p><div className="mt-3 flex flex-wrap gap-2"><StatusChip tone={participation ? "lime" : "sun"}>{participation ? "Aktiveras" : "Utkast"}</StatusChip><StatusChip tone={draft.inventoryPolicy === "tracked" ? "electric" : "neutral"}>{participation ? "Aktivitetsbiljett" : draft.inventoryPolicy === "tracked" ? `${matrix.length} lagerspårade varianter` : "Utan lagersaldo"}</StatusChip>{pendingImages.length ? <StatusChip tone="electric">{pendingImages.length} bilder</StatusChip> : null}</div></div></div><div className="mt-5 grid grid-cols-2 gap-2"><Metric label="Baspris" value={sek(Number(draft.price))} /><Metric label="Moms" value={`${draft.vatRate}%`} /><Metric label="Kanal" value={participation ? "Aktivitet" : draft.inventoryPolicy === "tracked" || draft.standaloneEnabled ? "Fristående shop" : draft.activityAddonEnabled ? "Aktivitet" : "Ej till salu"} /><Metric label="Leverans" value={participation ? "Deltagande" : draft.inventoryPolicy === "tracked" || draft.fulfillment === "desk_pickup" ? "Hämtas i receptionen" : draft.fulfillment} /></div>{participation ? <div className="mt-3 rounded-xl p-3 text-xs" style={{ background: ax("surface") }}><p className="font-black text-white">Founder {draft.founderDiscount || 0}% · Standard 12h</p><p className="mt-1" style={{ color: ax("muted") }}>Day/Play/Play+ ingår inte som standard. Produkten aktiveras först efter att medlemsprisregeln har sparats.</p></div> : null}</section><aside className="space-y-3"><TrackedGate /><button type="button" onClick={() => create.mutate()} disabled={create.isPending || (participation && Number(draft.founderDiscount || 0) > 0 && membershipTiersQuery.isLoading)} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl text-sm font-black disabled:opacity-40" style={{ background: `linear-gradient(135deg, ${ax("electric")}, ${ax("magenta")})`, color: "white" }}>{create.isPending || (participation && membershipTiersQuery.isLoading) ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} {participation ? "Skapa aktivitetsprodukt" : "Skapa säkert utkast"}</button><p className="text-center text-[10px]" style={{ color: ax("muted") }}>{participation ? "Skapas i access_products och använder befintlig prissättning, entitlement och Policy V1." : "Bilderna laddas upp först när utkastet skapas. Ingen spårad försäljning aktiveras."}</p></aside></div> : null}
...
function ProductDetail({ venueId, product, products, relationships, operations, onBack, refreshAll, onLoadMoreOrders, ordersLoadingMore, onLoadMoreMovements, movementsLoadingMore }: { venueId: string; product: AdminCommerceProduct; products: AdminCommerceProduct[]; relationships: ProductRelationship[]; operations?: InventoryOperations; onBack: () => void; refreshAll: () => Promise<void>; onLoadMoreOrders?: () => void; ordersLoadingMore?: boolean; onLoadMoreMovements?: () => void; movementsLoadingMore?: boolean }) {
  ...
  const storefrontColorValues = detail?.options.find((option) => option.code === "color")?.product_option_values || [];
  return <div className="space-y-4" data-testid="commerce-product-detail"><div className="flex items-center gap-3"><button type="button" onClick={onBack} className="grid h-10 w-10 place-items-center rounded-xl" style={{ background: ax("surfaceHi"), color: "white" }} aria-label="Tillbaka till produkter"><ArrowLeft className="h-4 w-4" /></button><div className="min-w-0 flex-1"><p className="font-mono text-[10px] font-black uppercase tracking-[0.2em]" style={{ color: ax("magenta") }}>Commerce · Produkt</p><h2 className="truncate font-display text-xl font-black text-white">{product.name}</h2></div>{product.status !== "archived" ? <button type="button" onClick={() => { setDraft((current) => ({ ...current, status: current.status === "active" ? "draft" : "active" })); }} className="rounded-xl px-3 py-2 text-[10px] font-black" style={{ border: `1px solid ${ax("border")}`, color: draft.status === "active" ? ax("sun") : ax("lime") }}>{draft.status === "active" ? "Flytta till utkast" : "Publicera katalogstatus"}</button> : null}</div>
    ...
    {tab === "overview" ? <ProductMediaEditor venueId={venueId} productId={product.id} productName={draft.name || product.name} media={product.media || []} optionValues={storefrontColorValues} onChanged={refresh} /> : null}
    {tab === "overview" && product.commerce_kind === "merchandise" && product.standalone_enabled ? <StorefrontPresentationEditor venueId={venueId} product={product} detail={detail} onChanged={refresh} /> : null}
