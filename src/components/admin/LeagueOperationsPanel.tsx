...
import { isValidLeagueV1GameScore } from "@/lib/leagueRules";
...
function FixtureCard({ fixture, data, venueId, date }: { fixture: LeagueFixture; data: LeagueOperationsProjection; venueId: string; date: string }) {
  ...
  const completedSets = [0, 1, 2].flatMap((index) => {
    ...
    return teamA !== "" && teamB !== "" && isValidLeagueV1GameScore(Number(teamA), Number(teamB))
  ...
  const complete = [0, 1, 2].every((index) => isValidLeagueV1GameScore(Number(scores[index * 2]), Number(scores[index * 2 + 1])));
