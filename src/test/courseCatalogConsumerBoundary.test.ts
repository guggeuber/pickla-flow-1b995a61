...
describe("Course Catalog consumer boundary", () => {
  it("keeps discovery on the lean catalog and Prices on its own bounded read model", () => {
    ...
    expect(prices).toContain("fetchPublicPrices(slug)");
    expect(prices).not.toContain("fetchCoursePricingCatalog");
