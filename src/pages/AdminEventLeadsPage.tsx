...
export default function AdminEventLeadsPage() {
  ...
  const venue = (venues || []).find((item: any) => item.id === venueId);
    ...
    <div className="min-h-screen bg-background max-w-2xl mx-auto">
      <div className="sticky top-0 z-20 px-4 pt-4 pb-3" style={{ background: "hsl(var(--background))" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-display font-bold tracking-tight">Event Leads</h1>
            <p className="text-[10px] text-muted-foreground">{venue?.name || "Pickla"}</p>
          </div>
        </div>
      </div>
      <div className="px-4 pb-8 pt-1">
        {venueId ? <AdminEventLeads venueId={venueId} /> : <p className="text-sm text-muted-foreground">Välj venue först</p>}
      </div>
    </div>
  );
}
