...
function CourtCard({
  ...
  return (
    <motion.div
      ...
      className={`flex items-center justify-between rounded-2xl border-2 px-8 py-6 ${cfg.bg} ${cfg.border}`}
    >
      {/* Left block */}
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-3">
          <span
            className={`w-4 h-4 rounded-full flex-shrink-0 ${cfg.dot} ${
              court.status === "active" ? "animate-pulse" : ""
            }`}
          />
          <span
            className={`font-display font-bold tracking-widest ${cfg.text}`}
      ...
      {court.endsAt && (
        <div className="text-right pl-6">
          <p
            className={`font-display font-black tabular-nums ${cfg.text}`}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 mb-6 pb-5 border-b border-border">
        <span className={`w-4 h-4 rounded-full flex-shrink-0 ${accent}`} />
        <h2
      ...
      {/* Court cards — fill remaining height equally */}
      <div
        className="grid gap-4 flex-1"
        style={{ gridTemplateRows: `repeat(${courts.length}, 1fr)` }}
      >
        {courts.map((court, i) => (
  ...
  return (
    <div className="h-screen bg-background text-foreground flex flex-col overflow-hidden select-none">
      ...
      <div
        ...
        style={{
          gridTemplateColumns: sportTypes.length > 0
            ? `repeat(${sportTypes.length}, 1fr)`
            : "1fr",
        }}
      >
        {sportTypes.length === 0 ? (
          <div className="flex items-center justify-center">
            <p className="text-3xl font-display text-muted-foreground">Laddar banor…</p>
          </div>
        ) : (
