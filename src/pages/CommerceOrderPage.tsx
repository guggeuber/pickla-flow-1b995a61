      ...
      <main className="relative mx-auto max-w-lg">
        ...
        {!(purchaseConfirmed && hasParticipation) ? <section className="divide-y divide-black/10 border-y border-black/10">
          {lines.map((line) => {
            ...
            return <div key={line.id} className="flex items-center justify-between gap-3 py-4"><div><p className="font-bold">{lineName}{line.quantity > 1 ? ` · ${line.quantity} st` : ""}</p>{line.variant_snapshot?.options?.length ? <p className="text-xs font-semibold text-slate-700">{line.variant_snapshot.options.map((option) => option.value_label).join(" / ")} · SKU {line.sku}</p> : null}<p className="text-xs text-slate-500">{line.fulfillment_type === "desk_pickup" ? fulfillmentLabel(line.fulfillment_status, isCancelled) : isDayPassLine ? "Gäller hela dagen" : "Din plats"}</p></div><p className="font-black">{formatCommerceMoney(line.line_total_inc_vat_minor || line.unit_price_minor * line.quantity)}</p></div>;
          })}
        </section> : null}
