...
type PricingRule = { id: string; name: string; type: string; price: number; time_from?: string | null; time_to?: string | null };
type FirstVisitOffers = { is_first_time: boolean; items: Array<{ route: string }> };
...
export default function PricesMembershipPage() {
  ...
  const hourly = (pricing.data || []).filter((rule) => rule.type === "hourly");
  ...
  return <div className="min-h-dvh bg-[#fffaf7] text-neutral-950"><PicklaTopBar slug={slug} background="#fffaf7" /><main className="mx-auto max-w-md px-5 pb-16 pt-[calc(env(safe-area-inset-top,0px)+104px)]"><p className="text-[11px] font-black uppercase tracking-[0.2em] text-neutral-400">Priser & medlemskap</p><h1 className="mt-2 text-[34px] font-black leading-none tracking-[-0.04em]">Spela på ditt sätt.</h1>
    {loading ? <Loader2 className="mx-auto mt-12 h-6 w-6 animate-spin" /> : <div className="mt-8 space-y-8">
      {firstVisit.data?.is_first_time ? <section><PriceRow title={firstVisit.data.items.length ? "Första gången? Spela för 99 kr." : "Första gången? 165 kr, racket ingår — kom på Open Play ikväll."} description={firstVisit.data.items.length ? "Racket finns att låna." : null} action={() => navigate(introRoute)} /></section> : null}
      <section><h2 className="text-lg font-black">Dagspass</h2>{dayPasses.map((product) => <PriceRow key={product.id} title={product.name} description={product.description || "Spela Open Play hela dagen."} price={formatCommerceMoney(product.base_price_sek * 100)} action={() => navigate(`/today?v=${encodeURIComponent(slug)}`)} />)}</section>
      <section><h2 className="text-lg font-black">Medlemskap</h2>{(memberships.data || []).map((tier) => <PriceRow key={tier.id} title={tier.name} description={tier.description || "Medlemsförmåner och priser visas innan köp."} price={tier.monthly_price != null ? `${tier.monthly_price} kr/mån` : null} action={() => navigate(`/membership?v=${encodeURIComponent(slug)}`)} />)}</section>
      <section><h2 className="text-lg font-black">Banpriser</h2>{hourly.map((rule) => <PriceRow key={rule.id} title={rule.name || "Bana"} description={rule.time_from && rule.time_to ? `${String(rule.time_from).slice(0, 5)}–${String(rule.time_to).slice(0, 5)}` : "Pris per bana och timme."} price={`${rule.price} kr/tim`} action={() => navigate(`/book?v=${encodeURIComponent(slug)}`)} />)}</section>
      <section><h2 className="text-lg font-black">Kurser</h2>{(courses.data?.items || []).map((course) => <PriceRow key={course.id} title={course.name} description={course.format?.description} price={formatCommerceMoney(course.product.base_price_sek * 100)} action={() => navigate(`/course/${course.id}?v=${encodeURIComponent(slug)}`)} />)}</section>
      {punchCards.length ? <section><h2 className="text-lg font-black">Klippkort</h2>{punchCards.map((product) => <PriceRow key={product.id} title={product.name} description={product.description} price={formatCommerceMoney(product.base_price_sek * 100)} action={() => navigate(`/shop?v=${encodeURIComponent(slug)}`)} />)}</section> : null}
    </div>}
  </main></div>;
}
