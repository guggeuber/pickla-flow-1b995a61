...
export default function LeaguePage() {
  ...
  const register = useMutation({
    ...
    onSuccess: (response) => {
      if (response.pricing.quote_changed === true) toast.info(`Early Bird hann ta slut. Lagplatsen reserverades för ${formatCommerceMoney(Number(response.pricing.final_price_minor || 0))}.`);
    ...
    <div className="min-h-[100dvh] bg-white text-slate-950">
      ...
      <main className="mx-auto w-full max-w-2xl px-5 pb-40 pt-[calc(env(safe-area-inset-top,0px)+96px)]">
        ...
        <section className="mt-3 flex items-center justify-between rounded-2xl bg-[#fff2f7] px-5 py-4">
          <div><p className="text-xs font-black uppercase tracking-wide text-[#b41663]">Lagpris · båda spelarna</p>{league.pricing_reason === "early_bird" ? <p className="mt-1 text-xs font-bold text-slate-500">Early Bird · första {league.product.early_bird_slots} lag</p> : null}</div>
          <div className="text-right"><p className="text-xl font-black">{formatCommerceMoney(league.current_price_minor)}</p>{league.pricing_reason === "early_bird" ? <p className="text-xs text-slate-400 line-through">{formatCommerceMoney(league.product.base_price_sek * 100)}</p> : null}</div>
        </section>
        ...
        {league.season.fixtures_published_at ? <>
          <section className="mt-10"><h2 className="text-xl font-black">Tabell</h2><div className="mt-4 overflow-x-auto rounded-2xl border border-black/10"><table className="w-full min-w-[520px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th className="p-3">#</th><th className="p-3">Lag</th><th className="p-3 text-center">M</th><th className="p-3 text-center">V</th><th className="p-3 text-center">F</th><th className="p-3 text-center">Set +/-</th><th className="p-3 text-center">Poäng</th></tr></thead><tbody>{league.standings.map((row) => <tr key={row.team_entry_id} className="border-t border-black/10"><td className="p-3 font-black">{row.position}</td><td className="p-3 font-bold">{row.team_name}</td><td className="p-3 text-center">{row.matches_played}</td><td className="p-3 text-center">{row.wins}</td><td className="p-3 text-center">{row.losses}</td><td className="p-3 text-center">{row.set_difference > 0 ? "+" : ""}{row.set_difference}</td><td className="p-3 text-center font-black">{row.league_points}</td></tr>)}</tbody></table></div><p className="mt-2 text-xs text-slate-500">Särskiljning: League-poäng, inbördes möten vid exakt två lika lag, poängskillnad, gjorda poäng. Delad placering kan förekomma.</p></section>
